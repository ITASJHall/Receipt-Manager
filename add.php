...
<?php
 if (isset($_POST['add'])){
	...
	unset($_POST['add']);
	$add_array = array('location', 'cost', 'cost_before_tax', 'pst', 'gst', 'method_of_payment', 'points_spent', 'receipt_type', 'points_earned', 'savings_total', 'cashier', 'purchaser', 'time_purchased', 'items_id', 'transaction_id');
	...
	$exists = mysqli_query($conn,"SELECT * FROM  `_receipts` WHERE `tr_id` = '" . $transaction_id . "'");
	...
	if(empty($exists)) {
		...
		$query .= (!empty($transaction_id) ? "`tr_id` = '" . $conn->real_escape_string(htmlentities($transaction_id)) . "', " : "");
		$query .= "`time_stamp_updated` = NOW(), ";
		$query .= "`time_stamp_created` = NOW();";
		...
		echo "Receipt already entered with that Transaction ID";
	}
}
